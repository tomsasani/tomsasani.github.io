...
They made it clear that scientific knowledge is never perfect, that there's often room for different conclusions from the same data, and that even good science can be subject to valid criticism.
(were the data in Mendel's notebook a little too perfect, for example? []). 
As misinformation is now peddled by innumerable "bad actors," it's more important than ever to have a toolbox for critically evaluating scientific claims. 

This toolbox should include lessons on how to identify data visualizations that distort or mis-represent underlying data, and 
...
In general, I find that extended, long-term laboratory projects, supplemented with more frequent (but brief) assessments, provide an effective learning environment for many students. 
...
I've also found that short, anonymous quizzes (facilitated via smartphone apps, quiz websites, or anonymous quiz remotes) prior to each classroom session provide a low-stakes environment for assessing comprehension throughout a course. 
