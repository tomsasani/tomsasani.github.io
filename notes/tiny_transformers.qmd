...
Overall, the transformer exhibits higher accuracy than the CNNs on the recombination rate classification task (@fig-rho-performance).
...
Overall, the transformer exhibits higher accuracy than the CNNs on the three-population classification task, as well (@fig-demo-performance).
...
dfs = []
for fh in glob.glob("../data/*.tsv"):
    if not ("dry" in fh or "faithful" in fh or "giddy" in fh):
        continue
    df = pd.read_csv(fh, sep="\t")
    df["Model"] = df.apply(
        lambda row: (
