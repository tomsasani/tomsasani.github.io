...
We can do $g^{-1}(\beta_0 + \beta_{1}X_1)$ to ensure that our predictions match the expected distribution.
...
We do this by applying something called the "logit link."
More explicitly, we apply the inverse (g^-1) of the link to our lienar predictor (the mx + b part of the equation).
...
::: {.notes}
As a side note, for every distribution you might want to model, there is something called a "canonical" link function -- basically, a natural way to transform your mx + b predictions to match the distribution.
...
* An expected distribution for the $Y$ values

* A linear predictor

* A "link" between the our linear predictor and the response variable
...
```{r}
#| fig-cap: De novo mutation counts in Utah families. Data from Sasani et al. (2019).
library(dplyr)
...
::: {.callout-note}
# Note!
You can pick any link function you like, even if it's not the canonical or natural link &mdash; it depends on how you want to model $Y | X$!
:::
