...
format: 
    ...
    # revealjs:
    #     theme: default
# header-includes: \setbeameroption{show notes on second screen=right} \setbeamercolor{math text}{fg=green}
execute:
  echo: false
---
...
ggplot(galton_filt, aes(x=midparentHeight, y=childHeight)) +
    ...
    geom_segment(aes(xend = midparentHeight, yend = pred), color="red", alpha=0.5) +
    theme_cowplot() +
    labs(x="Mid-parent height", y="Child height") +
    theme(text = element_text(size = 9), axis.text.y = element_text(size=9), axis.text.x = element_text(size=9))
```
...
3. A "link" between the our linear predictor and the conditional mean of the response variable
...
ggplot(merged, aes(x=phase, y=n, col=phase)) +
    ...
    theme(legend.position="none", text = element_text(size = 9), axis.text.y = element_text(size=9), axis.text.x = element_text(size=9))
```
...
A "link" $g$ between the our linear predictor $\eta$ and the conditional mean $\mathbb{E}(Y | X) = \mu$ of the response variable.
...
**Note**: you can pick any link function you like, even if it's not the "canonical" or natural link &mdash; it depends on how you want to model $Y | X$!
